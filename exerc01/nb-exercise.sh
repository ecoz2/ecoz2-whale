...
function one_exercise() {
  ...
  ls data/sequences/TRAIN/M${M} | \
    parallel --results results "ecoz2 nbayes learn data/sequences/TRAIN/M${M}/{}"

  # If sequentially:
  #for class in `ls data/sequences/TRAIN/M${M}/`; do
  #  ecoz2 nbayes learn data/sequences/TRAIN/M${M}/${class} &
  #done
  #wait
  ...
  cmd="ecoz2 nbayes classify --models data/nbs/M${M} --sequences data/sequences/TRAIN/M${M}"
  echo "$cmd"
  $cmd
  ...
  cmd="ecoz2 nbayes classify --models data/nbs/M${M} --sequences data/sequences/TEST/M${M}"
  echo "$cmd"
  $cmd
